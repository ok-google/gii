...
<form class="ajax" data-action="{{ route('superuser.inventory.stock_adjusment.store') }}" data-type="POST" enctype="multipart/form-data">
  <div class="block">
    <div class="block-header block-header-default">
      <h3 class="block-title">Stock Adjusment</h3>
    </div>
    <div class="block-content">
      <div class="form-group row">
        <label class="col-md-3 col-form-label text-right" for="code">Code <span class="text-danger">*</span></label>
        <div class="col-md-7">
          <input type="text" class="form-control" id="code" name="code" onkeyup="nospaces(this)" value="{{ App\Repositories\StockAdjusmentRepo::generateCode() }}" readonly>
        </div>
      </div>
      <div class="form-group row">
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <div class="form-check">
            <input class="form-check-input" style="margin-top: 10px" type="checkbox" id="minus" name="minus">
          </div>
        </div>
      </div>
    </div>
    ...
    $('#add').on( 'click', function (e) {
      ...
      if(duplicate == 1) {
        ...
        if(product_id) {
          table.row.add([
                      counter,
                      '<input type="hidden" name="col_product_id[]" value="'+product_id+'"><span>'+sku+'</span>',
