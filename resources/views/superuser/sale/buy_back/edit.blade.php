...
<form class="ajax" data-action="{{ route('superuser.sale.buy_back.update', $buy_back->id) }}" data-type="POST" enctype="multipart/form-data">
  <input type="hidden" name="_method" value="PUT">
  <input type="hidden" name="ids_delete" value="">
  <div class="block">
    <div class="block-header block-header-default">
      <h3 class="block-title">Edit Buy Back</h3>
    </div>
    <div class="block-content">
      <div class="form-group row">
        <label class="col-md-3 col-form-label text-right" for="code">Code <span class="text-danger">*</span></label>
        <div class="col-md-7">
          <input type="text" class="form-control" id="code" name="code" onkeyup="nospaces(this)" value="{{ $buy_back->code }}">
        </div>
      </div>
      <div class="form-group row">
        <label class="col-md-3 col-form-label text-right" for="sales_order">Select Invoice <span class="text-danger">*</span></label>
        <div class="col-md-7">
          <select class="js-select2 form-control" id="sales_order" name="sales_order" data-placeholder="Select Invoice" data-value="{{ $buy_back->sales_order_id }}" disabled>
            <option></option>
            <option value="{{ $buy_back->sales_order_id }}" selected>{{ $buy_back->sales_order->code }}</option>
          </select>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <select class="js-select2 form-control" id="warehouse" name="warehouse" data-placeholder="Select Inventory">
            @foreach($warehouses_display as $item)
              <option></option>
              <option value="{{ $item->id }}" {{ $buy_back->warehouse_id == $item->id ? 'selected': '' }}>{{ $item->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <div class="form-check">
            <input class="form-check-input" style="margin-top: 10px" type="checkbox" id="disposal" name="disposal" {{ $buy_back->disposal == 1 ? 'checked': '' }}>
          </div>
        </div>
      </div>
    </div>
        ...
        <tbody>
          @foreach ($buy_back->details as $item)
              <tr>
                ...
                <td>
                  <input type="hidden" name="col_so_detail[]" value="{{ $item->sales_order_detail_id }}">
                  <input type="hidden" class="form-control" name="edit[]" value="{{ $item->id }}">
                  <span>{{ $item->sales_order_detail->product->code }}</span>
                </td>
                <td>
                  <span>{{ $item->sales_order_detail->product->name }}</span>
                </td>
                <td>
                  <span>{{ $item->sales_order_detail->price }}</span>
                </td>
                <td>
                  <input type="number" class="form-control" name="col_buy_back_price[]" min="1" value="{{ $item->buy_back_price }}" required>
                </td>
                <td>
                  <input type="number" class="form-control" name="col_buy_back_qty[]" min="1" max="{{ $item->sales_order_detail->quantity }}" placeholder="{{ $item->sales_order_detail->quantity }}" value="{{ $item->buy_back_qty }}" required>
                </td>
                <td>
                  <input type="number" class="form-control" name="col_buy_back_total[]" min="1" value="{{ $item->buy_back_total }}" required readonly>
                </td>
...
<script>
  $(document).ready(function () {
    ...
    $('#add').on( 'click', function (e) {
      ...
      if(duplicate == 1) {
        ...
        if(sales_order_detail_id) {
          table.row.add([
                      counter,
                      '<input type="hidden" name="col_so_detail[]" value="'+sales_order_detail_id+'"><input type="hidden" class="form-control" name="edit[]" value=""><span>'+sku+'</span>',
    ...
    let data_val = $('#sales_order').data('value')
    if (data_val > 0) {
      $('#sales_order').val(data_val);
      $('#sales_order').trigger('select2:select');
    }
