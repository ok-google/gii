...
<div class="block">
  <div class="block-header block-header-default">
    <h3 class="block-title">Edit Purchase Order (PPB)</h3>
  </div>
  <div class="block-content">
    <form class="ajax" data-action="{{ route('superuser.purchasing.purchase_order.update', $purchase_order->id) }}" data-type="POST" enctype="multipart/form-data">
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <textarea class="form-control" id="address" name="address">{{ $purchase_order->address }}</textarea>
        </div>
      </div>
      <div class="form-group row">
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7 text-right">
          <div class="col-md-3 form-check form-check-inline">
            <input class="form-check-input" type="radio" name="transaction_type" id="transaction_type1" value="1" {{ $purchase_order->transaction_type == '1' ? 'checked' : '' }}>
            <label class="form-check-label" for="transaction_type1">Tunai</label>
          </div>
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="transaction_type" id="transaction_type0" value="0" {{ $purchase_order->transaction_type == '0' ? 'checked' : '' }}>
            <label class="form-check-label" for="transaction_type0">Non Tunai</label>
          </div>
        </div>
      </div>
      <div class="form-group row" id="coa" {{ ($purchase_order->coa_id == null) ? 'style=display:none' : '' }}>
        ...
        <div class="col-md-7">
          <select class="js-select2 form-control" id="coa" name="coa" data-placeholder="Select Cash">
            <option></option>
            @foreach($coas as $coa)
            <option value="{{ $coa->id }}" {{ ($coa->id == $purchase_order->coa_id) ? 'selected' : '' }}>{{ $coa->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-3">
          <input type="text" class="form-control" id="kurs" name="kurs" value="{{ $purchase_order->kurs }}">
        </div>
      </div>
      <div class="form-group row">
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <select class="js-select2 form-control" id="local_freight" name="local_freight" data-placeholder="Select Ekspedisi">
            <option></option>
            @foreach($ekspedisis as $ekspedisi)
            <option value="{{ $ekspedisi->id }}" {{ $purchase_order->local_freight == $ekspedisi->id ? 'selected' : '' }}>{{ $ekspedisi->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
      ...
      <div class="form-group row pt-30">
        <div class="col-md-6">
          <a href="{{ route('superuser.purchasing.purchase_order.step', ['id' => $purchase_order->id]) }}">
