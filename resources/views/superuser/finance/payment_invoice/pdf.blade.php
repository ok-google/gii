<html>
  <head>
    ...
    <title>{{ $payment_invoice->code }}</title>
  ...
  <body style="font-size: 0.7em;">
            ...
            <tr>
              <td style="width: 50%">
                <table style="border-collapse: collapse">
                  ...
                  <tr><td class="border-full text-center">{{ $payment_invoice->code }}</td></tr>
                </table>
              </td>
              <td style="width: 50%">
                <table style="border-collapse: collapse">
                  <tr><td class="border-full text-center">Tgl. Pembayaran</td></tr>
                  <tr><td class="border-full text-center">{{ \Carbon\Carbon::parse($payment_invoice->select_date)->format('d/m/Y') }}</td></tr>
                </table>
              </td>
            </tr>
          </table>
    ...
    <table>
      <tr>
        <td style="width: 45%" valign="top">
          <table style="border-collapse: collapse">
            <tr><td class="border-full" style="width: 20%">Pemasok</td><td class="border-full">{{ $payment_invoice->supplier->name }}</td></tr>
          </table>
        ...
        <td style="width: 15%" valign="top">
          ...
          <table style="border-collapse: collapse; margin-top: 5px">
            ...
            <tr><td class="border-full text-center">{{ $payment_invoice->coa->name }}</td></tr>
          </table>
          <table style="border-collapse: collapse; margin-top: 5px">
    ...
    <table class="mt-10" style="border-collapse: collapse">
      ...
      <tbody>
        @php
          $no = 1;
          $totalDetail = sizeof($payment_invoice->details);
        @endphp
        ...
        <tr>
          ...
          <td class="border-left border-right{{ $borderBottom }}">{{ $detail->ppb->code }}</td>
          <td class="border-left border-right{{ $borderBottom }}">{{ \Carbon\Carbon::parse($detail->ppb->created_at)->format('d/m/Y') }}</td>
          <td class="border-left border-right{{ $borderBottom }}">{{ \Carbon\Carbon::parse($detail->ppb->created_at)->format('d/m/Y') }}</td>
          <td class="border-left border-right text-right{{ $borderBottom }}">0</td>
          <td class="border-left border-right text-right{{ $borderBottom }}">{{ number_format($detail->ppb->grand_total_idr, 2, ',', '.') }}</td>
    ...
    <table class="mt-10" style="border-collapse: collapse; width: 45%">
      ...
      <tr>
        <td class="border-full">@php echo $payment_invoice->description ? $payment_invoice->description : "<br /><br /><br />" @endphp</td>
      </tr>
    </table>
