...
<form class="ajax" data-action="{{ route('superuser.finance.payment_invoice.update', $payment_invoice->id) }}" data-type="POST" enctype="multipart/form-data">
  <input type="hidden" name="_method" value="PUT">
  <input type="hidden" name="ids_delete" value="">
  <div class="block">
    <div class="block-header block-header-default">
      <h3 class="block-title">Edit Cash/Bank Payment (Inv)</h3>
    </div>
    <div class="block-content">
      <div class="form-group row">
        <label class="col-md-3 col-form-label text-right" for="code">Code <span class="text-danger">*</span></label>
        <div class="col-md-7">
          <input type="text" class="form-control" id="code" name="code" onkeyup="nospaces(this)" value="{{ $payment_invoice->code }}">
        </div>
      </div>
      <div class="form-group row">
        <label class="col-md-3 col-form-label text-right" for="select_date">Select Date <span class="text-danger">*</span></label>
        <div class="col-md-4">
          <input type="date" class="form-control" id="select_date" name="select_date" value="{{ $payment_invoice->select_date ? date('Y-m-d', strtotime($payment_invoice->select_date)) : '' }}">
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <select class="js-select2 form-control" id="coa" name="coa" data-placeholder="Select COA">
            @foreach($coa as $item)
              <option></option>
              <option value="{{ $item->id }}" {{ $payment_invoice->coa_id == $item->id ? 'selected': '' }}>{{ $item->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <select class="js-select2 form-control" id="supplier" name="supplier" data-placeholder="Select Supplier" data-value="{{ $payment_invoice->supplier_id }}" disabled>
            @foreach($suppliers as $item)
              <option></option>
              <option value="{{ $item->id }}" {{ $payment_invoice->supplier_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <textarea class="form-control" id="address" name="address" readonly>{{ $payment_invoice->supplier->address }}</textarea>
        </div>
      </div>
      <div class="form-group row">
        <label class="col-md-3 col-form-label text-right" for="note">Note</span></label>
        <div class="col-md-7">
          <textarea class="form-control" id="note" name="note">{{ $payment_invoice->description }}</textarea>
        </div>
      </div>
    </div>
    <hr>
    ...
    <div class="block-content">
      <table id="datatable" class="table table-striped table-vcenter table-responsive">
        ...
        <tbody>
          @foreach ($payment_invoice->details as $item)
              <tr>
                <td>{{ $loop->iteration }}</td>
                <td><input type="hidden" name="ppb_id[]" value="{{ $item->ppb_id }}"><span>{{ $item->ppb->code }}</span></td>
                <td>
...
<script>
  $(document).ready(function () {
    ...
    let data_val = $('#supplier').data('value')
    if (data_val > 0) {
      $('#supplier').val(data_val);
      $('#supplier').trigger('select2:select');
    }
