<html>
  ...
  <body style="font-size: 0.7em;">
    ...
    <table class="mt-25">
      <tr>
        <td class="text-center border-top border-bottom" style="font-size: 1.5em; padding: 10px"><strong>Cash/Bank Payment</strong></td>
      </tr>
    </table>
    ...
    <div class="mt-10 mb-25" style="width: 40%; float: right">
      <table>
        <tr>
          <td>No. Transaksi</td><td>:</td><td>{{ $payment->code }}</td>
        </tr>
        <tr>
          <td>Tanggal</td><td>:</td><td>{{ \Carbon\Carbon::parse($payment->select_date)->format('d/m/Y') }}</td>
        </tr>
      </table>
    </div>
    ...
    <table class="mt-25 main-table" style="border-collapse: collapse">
      ...
      <tbody>
        @php
          ...
          $kredit = 0;
        @endphp
        @foreach ($payment_debet as $detail)
        <tr>
        ...
        @endforeach
        @foreach ($payment_credit as $detail)
        <tr>
          <td>{{ $detail->coa->code }} {{ $detail->coa->name }}</td>
          <td>{{ $detail->name }}</td>
          <td class="text-right"></td>
