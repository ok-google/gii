...
class MutationDisplayController extends Controller
{
    ...
    public function search_sku(Request $request)
    {
        ...
        $products = Product::where('code', 'LIKE', '%' . $request->input('q', '') . '%')->get(['id', 'code as text', 'name']);
            ...
            try {
                ...
                if ($mutation_display->save()) {
                    foreach ($mutation_display->details as $item) {
                        // $stock_sales_order = Receiving::where('warehouse_id', $mutation_display->warehouse_from)
                        
                        // ->where('product_id', $item->product_id)->first();

                        $sql = ReceivingDetailColly::leftJoin('receiving_detail', 'receiving_detail_colly.receiving_detail_id', '=', 'receiving_detail.id')
                        ->leftJoin('receiving', 'receiving_detail.receiving_id', '=', 'receiving.id')
                        ->where('receiving_detail.product_id', $item->product_id)->where('receiving.warehouse_id', $mutation_display->warehouse_from)
                        ->select(DB::raw('sum(receiving_detail_colly.quantity_ri) as stock'))->first();
                        // dd($sql);
                        if ($sql->stock >= $item->qty) {
                            ...
                            // $stock_sales_order_to = Receiving::where('warehouse_id', $mutation_display->warehouse_to)->where('product_id', $item->product_id)->first();
                            ...
                            // } else {
                            //     $stock_sales_order_to = new ReceivingDetailColly;
