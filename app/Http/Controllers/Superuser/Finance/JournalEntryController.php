...
class JournalEntryController extends Controller
{
    ...
    {
        if (!Auth::guard('superuser')->user()->can('journal entry-manage')) {
            return abort(403);
        }
        ...
        $data['journal_setting'] = JournalSetting::where('status', 1)->get();

        return view('superuser.finance.journal_entry.index', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            if ($validator->passes()) {
                ...
                try {
                    ...
                    $response['redirect_to'] = route('superuser.finance.journal_entry.index');
