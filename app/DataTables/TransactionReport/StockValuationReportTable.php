...
class StockValuationReportTable extends Table
{
    ...
    private function query(Request $request)
    {
        ...
        (
            (SELECT SUM(receiving_detail.total_quantity_ri - (SELECT SUM(receiving_detail_colly.quantity_recondition) FROM receiving_detail_colly WHERE receiving_detail_colly.receiving_detail_id = receiving_detail.id)) AS receiving_qty FROM receiving_detail JOIN receiving ON receiving_detail.receiving_id = receiving.id WHERE master_products.id = receiving_detail.product_id AND receiving.status = 2 AND receiving.acc_at < "'.$request->start_date .' 00:00:00")
            -
            IFNULL((SELECT SUM(sales_order_detail.quantity) AS sale_qty FROM sales_order_detail JOIN sales_order ON sales_order_detail.sales_order_id = sales_order.id WHERE master_products.id = sales_order_detail.product_id AND sales_order.status = 2 AND sales_order_detail.hpp_total IS NOT NULL AND sales_order.acc_at < "'.$request->start_date .' 00:00:00"), 0)
            +
            IFNULL((SELECT SUM(sale_return_detail.quantity) AS return_qty FROM sale_return_detail JOIN sale_return ON sale_return_detail.sale_return_id = sale_return.id WHERE master_products.id = sale_return_detail.product_id AND sale_return.status = 2 AND sale_return.updated_at < "'.$request->start_date .' 00:00:00"), 0)
        ) AS opening_qty,
         
        (
            (SELECT SUM((receiving_detail.total_quantity_ri - (SELECT SUM(receiving_detail_colly.quantity_recondition) FROM receiving_detail_colly WHERE receiving_detail_colly.receiving_detail_id = receiving_detail.id)) * (SELECT SUM(ppb_detail.total_price_idr / ppb_detail.quantity) FROM ppb_detail WHERE ppb_detail.id = receiving_detail.ppb_detail_id)) AS total_receiving FROM receiving_detail JOIN receiving ON receiving_detail.receiving_id = receiving.id WHERE master_products.id = receiving_detail.product_id AND receiving.status = 2 AND receiving.acc_at < "'.$request->start_date .' 00:00:00") 
            - 
            IFNULL((SELECT SUM(sales_order_detail.hpp_total) AS total_sale FROM sales_order_detail JOIN sales_order ON sales_order_detail.sales_order_id = sales_order.id WHERE master_products.id = sales_order_detail.product_id AND sales_order.status = 2 AND sales_order.acc_at < "'.$request->start_date .' 00:00:00"), 0)
            +
            IFNULL((SELECT SUM(sale_return_detail.quantity * sale_return_detail.hpp) AS total_return FROM sale_return_detail JOIN sale_return ON sale_return_detail.sale_return_id = sale_return.id WHERE master_products.id = sale_return_detail.product_id AND sale_return.status = 2 AND sale_return.updated_at < "'.$request->start_date .' 00:00:00"), 0)
        ) AS opening_balance,

        (SELECT SUM(ppb_detail.quantity) AS purchase_qty FROM ppb_detail JOIN ppb ON ppb_detail.ppb_id = ppb.id WHERE master_products.id = ppb_detail.product_id AND ppb.status = 2 AND ppb.acc_at BETWEEN "'.$request->start_date .' 00:00:00" AND "'.$request->end_date .' 23:59:59" ) purchase_qty,

        (SELECT SUM(ppb_detail.total_price_idr) AS total_purchase FROM ppb_detail JOIN ppb ON ppb_detail.ppb_id = ppb.id WHERE master_products.id = ppb_detail.product_id AND ppb.status = 2 AND ppb.acc_at BETWEEN "'.$request->start_date .' 00:00:00" AND "'.$request->end_date .' 23:59:59" ) total_purchase,

        (SELECT SUM(receiving_detail.total_quantity_ri  - (SELECT SUM(receiving_detail_colly.quantity_recondition) FROM receiving_detail_colly WHERE receiving_detail_colly.receiving_detail_id = receiving_detail.id)) AS receiving_qty FROM receiving_detail JOIN receiving ON receiving_detail.receiving_id = receiving.id WHERE master_products.id = receiving_detail.product_id AND receiving.status = 2 AND receiving.acc_at BETWEEN "'.$request->start_date .' 00:00:00" AND "'.$request->end_date .' 23:59:59" ) receiving_qty,

        (SELECT 
        SUM((receiving_detail.total_quantity_ri - (SELECT SUM(receiving_detail_colly.quantity_recondition) FROM receiving_detail_colly WHERE receiving_detail_colly.receiving_detail_id = receiving_detail.id)) * (SELECT SUM(ppb_detail.total_price_idr / ppb_detail.quantity) FROM ppb_detail WHERE ppb_detail.id = receiving_detail.ppb_detail_id)) AS total_receiving 
        FROM receiving_detail 
        JOIN receiving ON receiving_detail.receiving_id = receiving.id
        WHERE master_products.id = receiving_detail.product_id AND receiving.status = 2 AND receiving.acc_at BETWEEN "'.$request->start_date .' 00:00:00" AND "'.$request->end_date .' 23:59:59") 
        total_receiving,
